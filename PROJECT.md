...
Environment variables are managed separately for local development and production deployments. This ensures your local dev environment and Vercel production deployment work independently without manual switching.

**📖 For detailed setup instructions, see [ENV_SETUP.md](frontend/ENV_SETUP.md)**

### Quick Reference

| Variable | Where to Set | Development | Production |
|----------|-------------|-------------|------------|
| `VITE_CONVEX_URL` | `.env.local` / Vercel | Dev Convex URL | Prod Convex URL |
| `VITE_CONVEX_SITE_URL` | `.env.local` / Vercel | Dev Convex Site | Prod Convex Site |
| `VITE_CLOUDINARY_*` | `.env.local` / Vercel | Your values | Same or separate |
| `SITE_URL` | Convex Dashboard | `http://localhost:5173` | Vercel URL |
| `GEMINI_API_KEY` | Convex Dashboard | Your key | Same or separate |
...
2. **Run Convex dev** (auto-generates Convex URLs):
   ```bash
   npx convex dev
   ```
...
4. **Set Convex backend variables:**
   - Go to [Convex Dashboard](https://dashboard.convex.dev) → Settings → Environment Variables
   - Add: `SITE_URL` = `http://localhost:5173`
   - Optional: `GEMINI_API_KEY` for AI features

**See [ENV_SETUP.md](frontend/ENV_SETUP.md#local-development-setup) for detailed steps.**

### Production (Vercel) Setup

1. **Create production Convex deployment** (recommended):
   ```bash
   cd frontend
   npx convex deploy
   ```
...
2. **Set environment variables in Vercel:**
   ...
   - Enable for: Production, Preview (and Development if desired)

3. **Set Convex backend variables:**
   - Go to Convex Dashboard → Settings → Environment Variables
   - Add: `SITE_URL` = `https://your-vercel-app.vercel.app` (your actual Vercel URL)
   - This enables CORS for your production frontend

4. **Redeploy Vercel** after adding variables

**See [ENV_SETUP.md](frontend/ENV_SETUP.md#production-vercel-setup) for detailed steps.**

### Required Variables

#### Frontend (Vite) - Set in `.env.local` (dev) or Vercel Dashboard (prod)

**Convex:**
- `VITE_CONVEX_URL` - Convex deployment URL
- `VITE_CONVEX_SITE_URL` - Convex site URL (for Better Auth)
...
**Optional:**
- `VITE_GEMINI_API_KEY` - For AI-powered media analysis

#### Convex Backend - Set in Convex Dashboard

**Required:**
- `SITE_URL` - Frontend URL for CORS (localhost for dev, Vercel URL for prod)

**Optional:**
- `GEMINI_API_KEY` - For server-side AI features

### Important Notes

- **Never commit `.env.local`** - It's git-ignored and contains secrets
- **Use separate Convex deployments** - Keep dev and production data isolated
- **VITE_ prefix required** - Only variables prefixed with `VITE_` are exposed to the browser
- **No code changes needed** - Environment variables are read automatically via `import.meta.env.VITE_*`
- **CORS configuration** - `SITE_URL` in Convex must match your frontend URL (localhost or Vercel)

### Troubleshooting

**CORS errors on Vercel?**
- Verify `SITE_URL` in Convex Dashboard matches your Vercel deployment URL
- Check that `VITE_CONVEX_SITE_URL` in Vercel matches your Convex deployment

**Variables not updating?**
- Vercel: Redeploy after adding/changing variables
- Convex: Changes take effect immediately (may take a few seconds)

**See [ENV_SETUP.md](frontend/ENV_SETUP.md#troubleshooting) for more troubleshooting tips.**
...
- Set all `VITE_*` variables in Vercel → Settings → Environment Variables
- See [ENV_SETUP.md](frontend/ENV_SETUP.md#production-vercel-setup) for detailed steps

**Backend Variables (Convex Dashboard):**
1. Go to Convex Dashboard → Settings → Environment Variables
2. Add production values:
   - `SITE_URL` = Your Vercel deployment URL (required for CORS)
   - `GEMINI_API_KEY` = Your Gemini API key (if using AI features)
   - Other server-side variables as needed

**Note:** Frontend variables (`VITE_*`) are set in Vercel, backend variables are set in Convex Dashboard.
...
**Important:**
- Enable variables for: Production, Preview (and Development if desired)
- After adding variables, redeploy your project
- See [ENV_SETUP.md](frontend/ENV_SETUP.md) for detailed instructions

**5. Configure Convex Backend for CORS**

In Convex Dashboard → Settings → Environment Variables:
- Add: `SITE_URL` = `https://your-vercel-app.vercel.app` (your actual Vercel deployment URL)
- This allows Better Auth to accept requests from your Vercel deployment
